...
func TestMemoryRollingSummaryUpsertGetClear(t *testing.T) {
	...
	// Clear 清空会话时联动删除摘要
	if _, err := s.Append("c1", Message{Role: RoleUser, Content: "hi"}); err != nil {
		t.Fatal(err)
	}
	s.Clear("c1")
	if _, ok := s.GetRollingSummary("c1"); ok {
...
func TestMemoryRollingSummaryForkDoesNotCopy(t *testing.T) {
	s := NewMemoryStore()
	m1, err := s.Append("c1", Message{Role: RoleUser, Content: "first"})
	if err != nil {
		t.Fatal(err)
	}
	if _, err := s.Append("c1", Message{Role: RoleUser, Content: "second"}); err != nil {
		t.Fatal(err)
	}
	if err := s.UpsertRollingSummary(RollingSummary{ConversationID: "c1", Summary: "S", CoversThroughMessageID: m1.ID, CoversThroughOrder: 0}); err != nil {
		t.Fatal(err)
	}
