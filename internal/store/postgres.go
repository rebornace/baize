...
CREATE TABLE IF NOT EXISTS runs (
  ...
  forced_tool_name TEXT, forced_tool_args_json TEXT
);
...
func OpenPostgres(dsn string) (*SQLStore, error) {
	...
	if _, err := db.Exec(`ALTER TABLE runs ADD COLUMN IF NOT EXISTS forced_tool_name TEXT`); err != nil {
		_ = db.Close()
		return nil, fmt.Errorf("migrate runs forced_tool_name: %w", err)
	}
	if _, err := db.Exec(`ALTER TABLE runs ADD COLUMN IF NOT EXISTS forced_tool_args_json TEXT`); err != nil {
		_ = db.Close()
		return nil, fmt.Errorf("migrate runs forced_tool_args_json: %w", err)
	}
