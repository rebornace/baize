...
export const zhPack = {
  ...
  // 侧栏 / 壳层
  closeMenu: '关闭菜单',
  conversationListAria: '对话列表',
  newChat: '新对话',
  scopeAria: '会话范围',
  scopeAll: '全部',
  scopeMine: '我的',
  deleteConversationTitle: '删除对话',
  deleteConversationAria: (label: string) => `删除对话 ${label}`,
  linkSettings: '设置',
  linkAccounts: '账号',
  logout: '退出',
  ...
  // Composer
  send: '发送',
  attachmentsAria: '附件列表',
  removeAttachmentAria: (name: string) => `移除 ${name}`,
  addAttachment: '添加附件',
  messageInputAria: '消息输入',
  skillCompleteAria: '技能补全',
  ...
  fileKindDeck: '演示',
  fileKindFile: '文件',
  toolRunning: (l: string) => `正在处理：${l}…`,
  toolWaitingHuman: (l: string) => `待确认：${l}`,
  toolSucceeded: (l: string) => `已完成：${l}`,
  toolFailed: (l: string) => `处理失败：${l}`,
  toolApproved: (l: string) => `已同意：${l}`,
  toolRejected: (l: string) => `已拒绝：${l}`,
  analysisPreviewTitle: '分析页预览',
  analysisPreviewLoading: '加载分析页…',
  ...
  restarting: '正在重启…',
  developer: '技术信息',
  developerConfig: (path: string) => `配置：${path}`,
  developerOverlay: (path: string) => `覆盖：${path}`,
  developerRunning: (driver: string) => `运行中：${driver}`,
  developerMismatch: '配置与运行中存储不一致',
},
  WEBHOOKS: {
  title: '消息回调',
  ...
  sectionDecide: '决策层',
  decideHint:
    '用便宜快速的小模型做高频结构化判断（枚举裁决），降低主模型 token 与延迟。任一步失败都会安全回退，不影响主流程。',
  decideEnabled: '启用决策层',
  decideMemoryEnabled: '记忆抽取预判（DP-1）',
  decideMemoryEnabledHint: '调用主模型前先判断这一轮是否值得抽取记忆。',
  fieldDecideProfile: '决策模型',
  hintDecideProfile: 'decide_profile_id · 留空表示跟随当前主模型',
  decideProfileNone: '不指定（跟随主模型）',
  decideToolRoutingEnabled: '工具候选收敛（DP-2a）',
  decideToolRoutingHint: '工具数量超过阈值时，先做两级路由收窄再交给主模型。',
  decideToolShadow: '影子模式（只观测，不真正收口）',
  decideToolShadowHint: '关闭后进入强制模式，真正用预筛结果替代完整工具集。',
  decideToolChoice: '枚举约束（DP-2b）',
  decideToolChoiceHint:
    '强制模式下让模型必须在收敛后的工具集中选择一个（tool_choice=required），减少乱选；影子模式下无效。',
  fieldDecideToolThreshold: '触发收敛的工具数量',
  hintDecideToolThreshold: 'decide_tool_threshold · 1–500',
  fieldDecideToolPreTopK: '关键词预筛保留数',
  hintDecideToolPreTopK: 'decide_tool_pre_topk · 1–500（建议 16）',
  fieldDecideToolTopK: '决策模型保留数',
  hintDecideToolTopK: 'decide_tool_topk · 1–200（影子模式下生效）',
  decideToolPruneEnabled: '工具结果瘦身（DP-3）',
  decideToolPruneHint: 'ReAct 过程中对超大工具结果按裁决压缩，避免后续提示词膨胀。',
  fieldDecidePruneThreshold: '触发瘦身的结果字符数',
  hintDecidePruneThreshold: 'decide_tool_prune_threshold · 1–100000',
  fieldDecidePruneMaxJudged: '单步最多裁决条数',
  hintDecidePruneMaxJudged: 'decide_tool_prune_max_judged · 1–100',
  decideRouteEnabled: '长文路由兜底（DP-4）',
  decideRouteHint: '确定性分类落在「标准」档且文本较长时，由决策层在快速/深度档之间裁决。',
  fieldDecideRouteMinRunes: '触发兜底的最短字符数',
  ...
  phModel: 'gpt-4o',
  phApiKeyCreate: '留空则使用环境变量',
  phApiKeyEdit: '留空则不修改',
  phApiKeyEnv: 'OPENAI_API_KEY',
  hintTier: '智能选择按对话难度在快速/标准/深度思考档位间选模型；图片消息只走勾选了「视觉」的模型。',
  hintContextTokens: '单位为 tokens；留空或 0 时使用默认 128000',
},
  SKILLS: {
  title: '技能',
  ...
  sourceManaged: '自动生成',
  loading: '加载中…',
  managedLoginTitle: (connectorId: string) => `连接器 ${connectorId} 的登录能力（自动维护）`,
  builtinTitles: {
    'data-analytics': '多源数据统计与完整交互分析页（筛选、下钻、导出 PDF）',
  ...
  phCaptureGlob: '*login*（填 __none__ 关闭）',
  ...
  errInvalidRequest: '填写内容不完整或不正确，请检查后重试。',
  errInternal: '服务暂时出了问题，请稍后重试。',
  groupMeta: (total: number, enabled: number) => `${total} 个工具 · ${enabled} 已启用`,
  enableAll: '全部启用',
  disableAll: '全部停用',
  noConnector: '（无 Connector）',
  collapse: '收起',
  displayName: '显示名',
  descriptionLabel: '说明',
  save: '保存',
  ...
  submitting: '提交中…',
  loading: '加载中…',
  addModalHintBefore: '此处仅添加单条 extra 工具；批量导入请用',
  addModalHintLink: '业务系统',
  addModalHintAfter: '上传接口文档。',
},
  CONNECTORS: {
  openapiTitle: '业务系统',
  openapiDesc:
  ...
  exportPolicyAria: (name: string) => `${name} 导出策略`,
  ...
  statusDelivered: '已发送',
  loading: '加载中…',
  runStatusPrefix: '运行状态：',
  runPolling: '轮询中',
  runStopped: '已停止',
  runReasonLoginAdmin: '（未登录：启用前请先扫码登录）',
  runReasonLoginOperator: '（未登录，请扫码登录）',
  runReasonLoginExpired: '（登录已过期：请重新扫码登录）',
  runReasonStartFailed: '（启动失败，请检查日志）',
  runReasonStoppedAdmin: '（适配器进程已手动停止，点「启动进程」恢复）',
  runReasonStoppedOperator: '（适配器进程已停止，请联系管理员启动）',
  processSection: '适配器进程',
  processStart: '启动进程',
  processRestart: '重启进程',
  processStop: '停止进程',
  processHint:
  ...
  qrScanHint: (url: string) => `请用手机微信扫码（内容：${url}）`,
  settingsSection: '设置',
  fieldAssignee: '受理人（assignee）',
  fieldAllowlist: 'allowlist（每行一个 peer id）',
  enableChannel: '启用微信渠道',
  saveSettings: '保存设置',
  saving: '保存中…',
  loginPending: '等待扫码…',
  loginSuccess: '登录成功',
  loginExpired: '二维码已过期，请重新获取',
  loginUnknown: '未知状态',
},
  INBOX: {
  title: '外部来信',
  ...
  SETTINGS_NAV: {
    ...
    mcpDesc: '接入标准 MCP 工具服务（本地子进程或远程 HTTP），扩展助手可用能力',
    plugins: '插件服务',
    pluginsDesc: '接入自行部署、按约定接口提供能力的程序，助手即可使用其能力',
    mcpExport: '对外提供能力',
  ...
  SETTINGS_BADGES: {
    modelsNone: '未配置',
    modelsCount: (n: number) => `已配置 ${n} 个`,
    toolsNone: '未启用',
    toolsAvailable: (n: number) => `${n} 项可用`,
    skillsCount: (n: number) => `${n} 个技能`,
    connectCta: '去接入',
    connectedCount: (n: number) => `已接 ${n} 个`,
    exportCount: (n: number) => `${n} 个出口`,
    weixinRunning: '运行中',
    weixinLoginRequired: '待登录',
    weixinLoginExpired: '登录已过期',
    weixinStartFailed: '启动异常',
    weixinStopped: '已停用',
    weixinNone: '未接入',
    webhookSet: '已设置',
    webhookUnset: '未设置',
    inboxCount: (n: number) => `${n} 个收件地址`,
    storeLocalFile: '本地文件',
    runtimeCustom: '已自定义',
    runtimeDefault: '默认',
  },
