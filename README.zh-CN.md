...
打开 `http://127.0.0.1:8080/ui`。若配置了控制面口令，打开 `/ui` 先解锁；操作员只能进账号，改助手功能需要管理员口令。WebUI 支持**中文 / English**，语言偏好仅保存在本机浏览器（`baize.locale`）。
...
2. 在 **设置 → 对外提供能力** 按工具配置导出策略（`default` / 强制允许 / 强制拒绝）。数据库 / MCP 写类工具永不导出。对接数据库类 MCP 时，可在 **设置 → 外部工具服务** 编辑该连接并勾选「对外导出时按数据库只读筛选」（`export_db_readonly`），进一步收紧导出面。
3. 生产环境请用 **HTTPS** 连接白泽。
