...
- 登录入口 @ 直达：后端从连接器 capture 配置实时推导登录入口、在 @ 弹窗作为特殊条目、绕过模型选择确定性直达登录工具；`login_required` 工具卡片显示「去登录」快捷入口。需改动 Run 工具调度与 @ 语义，单独立项设计。→ 见 `2026-09-14-ui-login-at-design.md`。
