...
- Settings → 账号记忆 (account memory): control-plane accounts can remember facts across conversations; manage entries on the settings page; auto-extraction can be turned off under Runtime settings
...
**HTTP — OAuth 2.1 (interactive)** — Some remote MCP servers require OAuth instead of a static API key in `mcp.headers`. Set **`runtime.public_base_url`** to the Runtime root URL that the admin browser can reach (authorization callback). Register the HTTP connector under **Settings → 外部工具服务 (external tool services)** (admin), optionally set a pre-registered `client_id`, then use **Authorize** / **Re-authorize** on the connector row (admin-only). Access and refresh tokens are sealed on the connector (`mcp.oauth`) and require **`BAIZE_SETTINGS_KEY`** in production — same encryption path as other persisted secrets (see **Settings encryption** below). On invoke, OAuth **`Authorization` overrides** a static header of the same name. **stdio MCP is unchanged** (credentials via `mcp.env` only).
...
- Each profile holds: name, Provider (fixed to `openai_compatible` in this release), Base URL, model name, API Key (or the API Key **environment variable name**), thinking level / dialect (with legacy `disable_thinking` migration), `supports_vision`, `context_tokens`, and an **Auto tier** (`light` / `standard` / `power`; pick "auto" to infer it from the model name). The tier tells the task-aware Auto router how capable the model is.
- **Thinking level & visible reasoning:** Chat can override thinking level per conversation; when the upstream returns reasoning tokens, Chat streams model thinking before the final reply (degrades cleanly if the gateway omits thinking).
